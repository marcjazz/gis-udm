...
- **Examples:**
  ...
  - Xen
  - KVM (Kernel-based Virtual Machine) - _Note: KVM turns the Linux kernel itself into a Type 1 hypervisor._
...
- **Advanced Topic: Nested Virtualization:** Explain that running a hypervisor _inside_ a VM (e.g., running Hyper-V inside a KVM guest) requires specific CPU extensions (`VT-x/AMD-V` must be exposed to the guest) and incurs a significant performance penalty.
...
- **Advanced Task:** Using the `VBoxManage` CLI tool (introduced in the Key Components section), attempt to list all virtual networks available on the host using `VBoxManage list natnets` or `VBoxManage list hostonlyifs`. This demonstrates interacting with the hypervisor outside the GUI.

---

## Further Reading

- [VMware ESXi Architecture Whitepaper](https://example.com/esxi_arch) (Placeholder for deeper dive)
- [Guide to Linux KVM Architecture](https://example.com/kvm_guide) (Placeholder for deeper dive)
