...
1.  **Boot Order:** The VM detects the bootable ISO in the virtual optical drive.
2.  **Installer:** Follow the OS installer's prompts.
3.  **Post-Install:** You **must "eject" the ISO** (unmount it) from settings so the VM boots from its virtual hard disk on the next restart.
