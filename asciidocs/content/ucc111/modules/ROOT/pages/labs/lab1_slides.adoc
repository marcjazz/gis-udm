...
:page-size: 16:9
:page-paginate: true
:page-header: UCC111-2: Linux Installation and Package Management
:page-footer: Lab 1: The Phoenix Server
...
. **Disaster Recovery (GRUB)**
. **`debootstrap` for Manual Installation**
...
== The Professional's Superpower: `chroot`
...
. **Boot** from a Live ISO (the `netinst` CD).
. **Mount** the broken system's partitions into a temporary location.
. **`chroot`** into the mounted root filesystem.
. **Re-install GRUB** using `grub-install`.
. **Reboot** the resurrected server.

This is a fundamental, resume-worthy skill.

'''

== The Expert Challenge: `debootstrap`
...
. Manually partition disks from a live CD.
. Mount them to a temporary directory.
. Run `debootstrap` to create the system files.
. `chroot` into the new system to install a kernel and configure it from scratch.
