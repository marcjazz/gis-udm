...
* link:https://cdimage.debian.org/debian-cd/current/amd64/iso-cd/debian-12.5.0-amd64-netinst.iso[Debian 12 Network Install ISO] downloaded.
...
... Partition the disk (`parted` or `fdisk`).
... Create the LVM structure (`pvcreate`, `vgcreate`, `lvcreate`).
... Format the filesystems (`mkfs.ext4`, `mkswap`).
... Mount everything under `/mnt` (e.g., root on `/mnt`, boot on `/mnt/boot`).
... Use *`debootstrap`* to install the Debian 'bookworm' release into `/mnt`.
... `chroot` into `/mnt` and manually install a kernel (`apt install linux-image-amd64`), GRUB (`apt install grub-pc`), configure `/etc/fstab`, set a root password (`passwd`), and create a user.
