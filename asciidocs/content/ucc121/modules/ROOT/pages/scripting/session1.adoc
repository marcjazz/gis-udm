...
. **Open `~/.bashrc`:** Use a text editor like `nano ~/.bashrc`.
. **Add Aliases:** Add the `ll` and `update` aliases shown in the examples above.
. **Add a Function:** Add the `mkcd` function.
. **Customize your Prompt:** Add the colored `PS1` export line from the example.
. **Activate Changes:** Save the file and run `source ~/.bashrc`.
. **Test Everything:**
