...
[NOTE]
====
While `#!/bin/sh` exists, it is often a stricter, POSIX-compliant shell. We use `#!/bin/bash` to unlock modern features like arrays and better string handling.
====
...
[NOTE]
====
You might see backticks \`date\` in old tutorials. Avoid them. `$()` is nestable and cleaner.
====
...
. Create a file named `start_project.sh`.
. Add the Shebang.
. Ask the user for a **Project Name**.
. Ask the user for an **Author Name**.
. Create a folder with the Project Name.
. Inside that folder, create a `README.md` file.
. Write the text "Project: [Name] created by [Author]" into the `README.md`.
...
* link:https://www.shellcheck.net/[ShellCheck.net]: The absolute best tool for finding bugs in your scripts. Paste your code there!
* link:https://explainshell.com/[Explainshell.com]: Type a command line to see a visual breakdown of what each flag does.
